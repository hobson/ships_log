...
function load_data_files(&$path_array) {
  ...
  foreach($path_array as $fi => $filepath) {
    ...
    if ( file_exists("$urlrelfilename") ) { // action HL: check for database entry, & merge the new stats with the old
      if ( strcmp($filepath,RelativePathPrefix.PictureFolder."$filename") ) { // rel path specified is dif from default one
      ...
      } // if file_exists($urlrelfilename...
    else { // if file_exists...
      if ((count($directories)>0)&&(strlen(end($directories))>0)&&(strcasecmp(end($directories),'.')!=0)) { // more than a basename or "./" was provided
